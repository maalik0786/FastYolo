...
namespace FastYolo
{
	public class YoloPreTrainedDatasetRepository
	{
		...
		public async Task<bool> DownloadDatasetAsync(string name, string destinationPath)
		{
			...
			var preTrainedData = _preTrainedDatas
				.Where(o => o.Name.Equals(name, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
			if (preTrainedData == null) return false;

			if (!await DownloadAsync(preTrainedData.ConfigFileUrl, destinationPath)
				.ConfigureAwait(false)) return false;

			if (!await DownloadAsync(preTrainedData.NamesFileUrl, destinationPath).ConfigureAwait(false)
			) return false;

			if (!await DownloadAsync(preTrainedData.WeightsFileUrl, destinationPath)
				.ConfigureAwait(false)) return false;
		...
		private async Task<bool> DownloadAsync(string url, string destinationPath)
		{
			...
			using (var httpClient = new HttpClient())
			{
				...
				using (var httpResponseMessage = await httpClient.GetAsync(url).ConfigureAwait(false))
				{
					...
					using (var sourceStream = new FileStream(filePath, FileMode.Append, FileAccess.Write,
